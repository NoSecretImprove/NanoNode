...
function numberTo64BytesBE(num) {
    const length = 64;
    const hex = num.toString(16).padStart(length * 2, '0');
    return hexToBytes(hex);
}
...
function sha512ModqLE(...args) {
    const hash = blake2.createHash(
      'blake2b',
      {
        digestLength: 64
      }
...
function getExtendedPublicKey(key) {
    key =
        typeof key === 'bigint' || typeof key === 'number'
            ? numberTo64BytesBE(normalizeScalar(key, MAX_512B))
            : ensureBytes(key);
    if (key.length !== 64)
...
function getPublicKey(privateKey) {
    return Buffer.from(getExtendedPublicKey(privateKey).pointBytes);
}
exports.getPublicKey = getPublicKey;
function sign(message, privateKey) {
    message = ensureBytes(message);
    const { prefix, scalar, pointBytes } =  getExtendedPublicKey(privateKey);
    const r = sha512ModqLE(prefix, message);
    const R = Point.BASE.multiply(r);
    const k = sha512ModqLE(R.toRawBytes(), pointBytes, message);
    const s = mod(r + k * scalar, CURVE.l);
    return Buffer.from(new Signature(R, s).toRawBytes());
}
exports.sign = sign;
function verify(sig, message, publicKey) {
    ...
    const k = sha512ModqLE(r.toRawBytes(), publicKey.toRawBytes(), message);
